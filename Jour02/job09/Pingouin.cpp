...
// Ajouter, modifier, supprimer et afficher les événements
void Pingouin::ajouterEvenement(const std::string& date, const std::string& description) {
    journal[date] = description;
    std::cout << "Événement ajouté le " << date << " : " << description << std::endl;
}

void Pingouin::modifierEvenement(const std::string& date, const std::string& nouvelleDescription) {
    if (journal.find(date) != journal.end()) {
        journal[date] = nouvelleDescription;
        std::cout << "Événement modifié le " << date << " : " << nouvelleDescription << std::endl;
    } else {
        std::cout << "Aucun événement trouvé à la date : " << date << std::endl;
    }
}

void Pingouin::supprimerEvenement(const std::string& date) {
    if (journal.erase(date)) {
        std::cout << "Événement supprimé à la date : " << date << std::endl;
    } else {
        std::cout << "Aucun événement trouvé à la date : " << date << std::endl;
    }
}
...
void Pingouin::afficherJournal() const {
    std::cout << "Journal des événements de " << nom << " :" << std::endl;
